...
int main()
{
    ...
    int rez = 0; //Переменная в которую заносится результат деления суммы на 10 без остатка
    int leftover = 0; //Переменная, в которую заносится остаток от деления вводимой суммы на 10 (если он есть)
    int altrez = 0; //Переменная, в которую заносятся операции с остатком, и которая используется для отображения кол-ва монет с окончанием, отличным от нуля
